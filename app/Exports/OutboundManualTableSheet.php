...
abstract class OutboundManualTableSheet extends OutboundManualSheet
{
    ...
    public function registerEvents(): array
    {
        return [AfterSheet::class => function (AfterSheet $event) {
            ...
            $lastColumn = $this->lastColumn();
            $count = $this->rowCount();
            $lastRow = max(self::HEADER_ROW, self::HEADER_ROW + $count);
            $range = 'A'.self::HEADER_ROW.':'.$lastColumn.$lastRow;

            $sheet->freezePane('A'.(self::HEADER_ROW + 1));
            $sheet->setAutoFilter($range);
            $borderRange = $count <= 5000 ? $range : 'A'.self::HEADER_ROW.':'.$lastColumn.self::HEADER_ROW;
            ...
            $sheet->getStyle('A'.self::HEADER_ROW.':'.$lastColumn.self::HEADER_ROW)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER)->setWrapText(true);
