...
class OutboundKoliExpectation
{
    /**
     * @return array{qty:int,koli:int,qty_per_koli:int}
     */
    public static function resolve(Item $item, ?int $qty = null, ?int $koli = null): array
    {
        $qty = $qty !== null ? (int) $qty : 0;
        ...
        if ($qty <= 0 && $koli === null) {
            throw ValidationException::withMessages([
                'items' => "Qty tidak valid untuk SKU {$sku}.",
            ]);
        }
        ...
        if ($qty % $qtyPerKoli !== 0) {
            throw ValidationException::withMessages([
                'items' => "Qty SKU {$sku} tidak bisa dibagi rata per koli. Isi/koli {$qtyPerKoli}, qty {$qty}.",
            ]);
        }
