...
class KpiDefinitionSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $rows = [
            ...
            ['Admin Outbound', 'Resi Import Duplicate Accuracy', 'Akurasi admin tidak melakukan import resi yang sudah pernah diimport.', '>=', 100, '%', 'daily', 'Otomatis', 'Ada', 'Resi Import', '(total resi upload - duplikasi no_resi) / total resi upload', 'Import resi melalui modul Resi Import. Sistem memeriksa no_resi duplikat pada periode snapshot sebagai indikator akurasi import.', 'Saat ini dihitung dari data duplikat no_resi di tabel resis. Jika perlu audit attempt yang diblokir, tambahkan log failed import attempt.'],
            ...
            ['Scan Outbound', 'Packing Employee Input Completion', 'Kelengkapan input karyawan yang melakukan packing pada proses scan out.', '>=', 100, '%', 'daily', 'Otomatis', 'Ada', 'Outbound > Scan Out, Scan Out History', 'scan out dengan packed_employee_id / total scan out', 'Operator scan out wajib memilih/input karyawan packer sebelum scan. Cek Scan Out History untuk paket tanpa packed by.', 'KPI ini menggantikan Courier Sorting Accuracy dan mengukur disiplin input packer.'],
