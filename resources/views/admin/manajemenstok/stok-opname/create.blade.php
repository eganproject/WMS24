...
@push('toolbar')
    @include('layouts.partials._toolbar', [
        ...
        'breadcrumbs' => ['Admin', 'Manajemen Stok', 'Stock Opname', 'Tambah'],
    ])
@endpush

@section('content')
<div class="content flex-row-fluid" id="kt_content">
    <form id="opname-form" action="{{ route('admin.manajemenstok.stok-opname.store') }}" method="POST">
        @csrf
        <div class="card">
            <div class="card-header border-0 py-5">
                <h3 class="card-title fw-bolder mb-0">Form Stock Opname</h3>
            ...
            <div class="card-body pt-0">
                ...
                <div class="row g-5">
                    <div class="col-xl-4">
                        <div class="fv-row">
                            ...
                            <input type="text" name="code" class="form-control form-control-white" value="{{ old('code', $newCode) }}" readonly>
                            <div class="form-text">Kode dokumen otomatis.</div>
                        </div>
                    </div>
                    <div class="col-xl-4">
                        <div class="fv-row">
                            <label class="required form-label">Tanggal Mulai</label>
                            <input type="text" name="start_date" class="form-control form-control-solid flatpickr-input @error('start_date') is-invalid @enderror" value="{{ old('start_date', date('Y-m-d')) }}" required>
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">Tanggal mulai proses stock opname.</div>
                        </div>
                    </div>
                    <div class="col-xl-4">
                        <div class="fv-row">
                            ...
                            @else
                                ...
                                @enderror
                            @endif
                            <div class="form-text">Pilih gudang yang akan dilakukan opname.</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-5 mt-1">
                    <div class="col-12">
                        ...
                        <textarea name="description" class="form-control form-control-solid" rows="2" placeholder="Catatan tambahan (opsional)">{{ old('description') }}</textarea>
                    </div>
                </div>
...
function addRow(){
    ...
    <tr data-index="${rowIndex}">
        ...
        <td><input type="number" step="any" name="items[${rowIndex}][system_quantity]" class="form-control form-control-white sys-qty" value="0" readonly></td>
        <td><input type="number" step="any" name="items[${rowIndex}][system_koli]" class="form-control form-control-white sys-koli" value="0" readonly></td>
        ...
        <td><input type="number" step="any" class="form-control form-control-white disc-qty" value="0" readonly></td>
        <td><input type="number" step="any" class="form-control form-control-white disc-koli" value="0" readonly></td>
...
$(document).ready(function(){
    ...
    $('#opname-form').on('submit', function(e){
        ...
        e.preventDefault();
        Swal.fire({
            text: 'Apakah Anda yakin ingin menyimpan data stock opname ini?',
            icon: 'question',
