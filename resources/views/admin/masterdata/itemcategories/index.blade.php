...
@push('toolbar')
    @include('layouts.partials._toolbar', [
        'title' => 'Kategori Item',
        'breadcrumbs' => ['Admin', 'Masterdata', 'Kategori Item'],
    ])
@endpush
            ...
            <div class="card-header border-0 pt-6">
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <!--end::Svg Icon-->
                        <input type="text" id="itemcategory_search_input" class="form-control form-control-solid w-250px ps-15"
                ...
                <div class="card-toolbar">
                    ...
                    @if ($canCreate)
                        <div class="d-flex justify-content-end">
                            <a href="{{ route('admin.masterdata.itemcategories.create') }}" class="btn btn-primary">Tambah Kategori Item</a>
                        </div>
                    @endif
                    <!--end::Toolbar-->
                </div>
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4"
                                                    ...
                                                    @if ($canEdit)
                                                        <div class="menu-item px-3">
                                                            <a href="{{ route('admin.masterdata.itemcategories.edit', $itemcategory->id) }}"
                                                    ...
                                                    @if ($canDelete)
                                                        <div class="menu-item px-3">
                                                            <form class="form-delete"
                                                                action="{{ route('admin.masterdata.itemcategories.destroy', $itemcategory->id) }}" method="POST"
                                                                style="display: inline-block;">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit"
                                                                    class="menu-link px-3 border-0 bg-transparent w-100 text-start"
                                                                    data-itemcategory-name="{{ $itemcategory->name }}">
                                                                    Delete
                                                                </button>
                                                            </form>
                                                        </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#itemcategory_search_input').on('keyup', function() {
            ...
            if (canDelete) {
                $('#table-on-page').on('submit', '.form-delete', function(e) {
                    ...
                    var form = $(this);
                    var n = form.find('button[data-itemcategory-name]').data('itemcategory-name');
