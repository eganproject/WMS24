    ...
    <div class="card-body p-6 p-lg-8">
        <div class="d-flex flex-column flex-xl-row justify-content-between gap-6">
            <div>
                ...
                <div class="text-muted">Rumus: rata-rata keluar/hari × periode forecast, lalu dikurangi stok saat ini. Hanya SKU dengan rata-rata keluar di atas 0 dan kebutuhan restock di atas 0 yang ditampilkan.</div>
            </div>
...
<div class="row g-5 mb-6">
    ...
    <div class="col-md-6 col-xl-3"><div class="card runout-card bg-light-primary"><div class="card-body"><div class="text-muted fw-semibold mb-2">Rata-rata Keluar / Hari</div><div class="fs-2 fw-bolder text-primary runout-number" id="summary_daily_average">0</div></div></div></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof $.fn.select2 !== 'undefined') {
        $(fields.warehouse).select2({ width: '100%' }); $(fields.category).select2({ placeholder: 'Semua Kategori', allowClear: true, width: '100%' });
    }

    const table = $table.DataTable({
        processing: true, serverSide: true, ordering: false, dom: 'rtip', pageLength: 25,
        ajax: { url: dataUrl, data: (params) => { params.warehouse_id = fields.warehouse.value; params.history_days = fields.history.value; params.forecast_days = fields.forecast.value; params.category_id = fields.category.value; params.q = fields.search.value; }, dataSrc: (json) => {
    ...
    [fields.warehouse, fields.category].forEach((field) => field.addEventListener('change', reloadFromFirstPage));
});
</script>
@endpush
