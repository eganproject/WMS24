...
@push('styles')
    <style>
        ...
        .doc-header { display:grid; grid-template-columns: 1fr 260px; gap:16px; align-items:center; margin-bottom:18px; }
        ...
        .table-doc th,.table-doc td{ border:1px solid var(--doc-border); padding:10px 12px; vertical-align:top; }
...
@push('toolbar')
    @include('layouts.partials._toolbar', [
        ...
        'breadcrumbs' => ['Admin', 'Stok Keluar', 'Permintaan Barang', 'Detail'],
    ])
@endpush
...
@section('content')
    <div class="content flex-row-fluid" id="kt_content">
        @php
            ...
            elseif ($status === 'on_progress' || $status === 'shipped') { $ribbonBg = 'warning'; }
            elseif ($status === 'approved') { $ribbonBg = 'info'; }
        @endphp
        <div class="card">
            ...
            <div class="card-body p-lg-12">
                <div class="doc-wrapper">
                    <div class="doc-header">
                        <div>
                            <div class="doc-title fs-2">PERMINTAAN BARANG</div>
                            <div style="color:var(--doc-muted); font-size:.95rem;">Dokumen Permintaan Barang Keluar</div>
                        </div>
                        <div class="doc-meta">
                            ...
                            @php
                                ...
                                elseif ($transferRequest->status === 'on_progress' || $transferRequest->status === 'shipped') { $badgeClass = 'warning'; }
                                elseif ($transferRequest->status === 'approved') { $badgeClass = 'info'; }
                            @endphp
                        ...
                        <table class="table-doc">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th style="text-align:right;">{{ number_format($totalQuantity ?? 0, 0, ',', '.') }}</th>
                                    <th style="text-align:right;">{{ number_format($totalKoli ?? 0, 2, ',', '.') }}</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    ...
                    @if(isset($transferRequest->shipments) && $transferRequest->shipments->count())
                        <div class="shipments-print-break" style="margin-top:36px;">
                            ...
                            @foreach ($transferRequest->shipments as $shipment)
                                @php
                                        ...
                                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $collapseId }}" aria-expanded="false" aria-controls="{{ $collapseId }}">
                                            <div class="d-flex w-100 justify-content-between align-items-center">
                                                ...
                                                <div class="text-end">
                                                    ...
                                                    <div class="meta-value">{{ number_format($shipQty ?? 0, 0, ',', '.') }} Qty • {{ number_format($shipKoli ?? 0, 2, ',', '.') }} Koli</div>
                                                    
                                                </div>
                                            </div>
                                        </button>
                                        <div class="ms-3 my-2 d-print-none">
                                            <a href="{{ route('admin.riwayat-pengiriman.show', ['shipment' => $shipment->id]) }}" target="_blank" class="btn btn-sm btn-light-primary">
                                                Cetak Surat Jalan
                                            </a>
                                        </div>
                                    </h2>
                                                    ...
                                                    <tbody>
                                                        @foreach ($shipment->itemDetails as $detail)
                                                            <tr>
                                                                ...
                                                                <td style="text-align:right;">{{ number_format($detail->quantity_shipped ?? 0, 0, ',', '.') }}</td>
                                                                <td style="text-align:right;">{{ number_format($detail->koli_shipped ?? 0, 2, ',', '.') }}</td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                                                    <tfoot>
                                                        <tr>
                                                            ...
                                                            <th style="text-align:right;">{{ number_format($shipQty ?? 0, 0, ',', '.') }}</th>
                                                            <th style="text-align:right;">{{ number_format($shipKoli ?? 0, 2, ',', '.') }}</th>
                                                        </tr>
                                                    </tfoot>
                                                </table>
                                            </div>
                    ...
                    <div class="d-flex justify-content-between mt-8 no-print">
                        <a href="{{ route('admin.stok-keluar.permintaan-barang.index') }}" class="btn btn-light">Kembali</a>
