...
@push('toolbar')
    @include('layouts.partials._toolbar', [
        'title' => 'Tambah Retur Out',
        'breadcrumbs' => ['Admin', 'Stok Keluar', 'Retur Out', 'Tambah'],
    ])
@endpush
...
<div class="content flex-row-fluid" id="kt_content">
  <div class="card">
    <div class="card-body">
      <form id="ri-form" action="{{ route('admin.stok-keluar.retur-out.store') }}" method="POST">
        @csrf
        <div class="row">
          <div class="col-md-4 mb-5">
            <label class="form-label required">Kode</label>
            <input type="text" name="code" class="form-control form-control-solid" value="{{ $code }}" readonly>
          </div>
          <div class="col-md-4 mb-5">
            <label class="form-label required">Tanggal Retur</label>
            <input type="text" name="return_date" id="return_date" class="form-control form-control-solid" value="{{ date('Y-m-d') }}">
          </div>
          <div class="col-md-4 mb-5">
        ...
        @if ($errors->has('details') || $errors->has('details.*'))
          <div class="alert alert-danger">Terdapat kesalahan pada daftar item. Periksa kembali kuantitas dan stok.</div>
        @endif
...
<script>
  $(function(){
    ...
    // initialize with a row if warehouse preselected
    if ($('#warehouse_id').val()) addRow();
