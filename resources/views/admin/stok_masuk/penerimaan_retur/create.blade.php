...
@push('toolbar')
    @include('layouts.partials._toolbar', [
        ...
        'breadcrumbs' => ['Admin', 'Stok Masuk', 'Penerimaan Retur', 'Tambah'],
    ])
@endpush
    ...
    <div class="card-body pt-6">
      @if ($errors->any())
        <div class="alert alert-danger mb-6">
          <ul class="mb-0 ps-5">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul>
        </div>
      @endif
      <form method="POST" action="{{ route('admin.stok-masuk.penerimaan-retur.store') }}" id="rr-form">@csrf
        <div class="row mb-7">
          <div class="col-md-4">
            <label class="form-label required">Kode</label>
            <input type="text" class="form-control form-control-solid" name="code" value="{{ $code }}" readonly>
          </div>
          <div class="col-md-4">
            <label class="form-label required">Tanggal</label>
            <input type="date" class="form-control form-control-solid flatpickr-date" name="return_date" value="{{ old('return_date', now()->toDateString()) }}">
          </div>
          <div class="col-md-4">
            ...
            <select name="warehouse_id" id="warehouse_id" class="form-select form-select-solid" data-control="select2" @if($forcedWarehouseId) disabled @endif>
              ...
              @foreach($warehouses as $w)
                <option value="{{ $w->id }}" @selected(old('warehouse_id', $forcedWarehouseId) == $w->id)>{{ $w->name }}</option>
              @endforeach
            </select>
        ...
        <div class="table-responsive">
          <table class="table align-middle" id="details-table">
            <thead><tr>
              ...
              <th class="text-end" style="width:15%">Koli</th>
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    function updateSummary(){
      ...
      $('#details-table tbody tr').each(function(){
        ...
        if ($(this).is(':visible')) { totalQty += q; totalKoli += k; count += 1; }
      });
    ...
    function addRow() {
      ...
      const tr = $(`
        <tr data-koli-ratio="1" data-name="">
          ...
          <td><input type="number" step="1" min="0" name="details[${idx}][quantity]" class="form-control form-control-solid text-end qty-input" value="0"></td>
    ...
    function isDuplicateSelection(val){
      let seen = 0; const v = String(val||'');
      $('#details-table tbody select[name$="[item_id]"]').each(function(){ if (String($(this).val())===v) seen++; });
      return seen > 1;
    }

    $('#add-row').on('click', addRow);
    $(document).on('click','.del-row', function(){ $(this).closest('tr').remove(); updateSummary(); });
    ...
    // Handle item change: set data-name and ratio, prevent duplicates
    $(document).on('change', 'select[name$="[item_id]"]', function(){
      const $row = $(this).closest('tr');
      const val = $(this).val();
      const meta = itemMap[String(val)];
      if (val && isDuplicateSelection(val)) {
      ...
      if (meta) {
        ...
        $row.data('koli-ratio', r); // keep jQuery cache in sync
      ...
      // Recalculate dependent field when changing item (ratio may change)
      ...
      if (!isNaN(qty) && qty > 0) {
        ...
        const newQty = Math.round(koli * ratio);
        $qty.val(newQty);
      }
      updateSummary();
    });

    // Mutual update between qty and koli using ratio
    $(document).on('input', '.qty-input', function(){
      const $row = $(this).closest('tr');
      const ratio = parseFloat($row.attr('data-koli-ratio')) || 1;
      $row.data('last-edited', 'qty');
    ...
    // Confirm submit and basic client-side validation
    $('#rr-form').on('submit', function(e){
      ...
      }).then((res)=>{ if(res.isConfirmed){ e.currentTarget.submit(); } });
    });
